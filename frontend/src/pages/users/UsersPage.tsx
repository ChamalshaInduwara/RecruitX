  ...
  */
  ...
  const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await api.post<UserActionResponse>("/users", {
        name: form.name.trim(),
  ...
  */
  ...
  const changeRole = async (userId: string, role: UserRole) => {
    try {
      ...
      await api.patch<UserActionResponse>(`/users/${userId}`, {
        role,
      });
      ...
      if (axios.isAxiosError(error)) {
        ...
        setActionError("Unable to change user role.");
      }
    } finally {
  ...
  */
  ...
  const toggleStatus = async (user: SystemUser) => {
    ...
    try {
      ...
      await api.patch<UserActionResponse>(`/users/${user.id}`, {
        status: newStatus,
      });
      ...
      if (axios.isAxiosError(error)) {
        ...
        setActionError("Unable to update account status.");
      }
    } finally {
      setUpdatingUserId(null);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Manage RecruitX staff, roles and account access.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <UserRoundCheck size={20} className="text-slate-500" />

          <p className="mt-3 text-xs text-slate-400">Active Users</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 text-xs text-slate-400">Recruiters</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 text-xs text-slate-400">Administrators</p>
                  ...
                  return (
                    <tr key={user.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-slate-700">
                        {user._count?.createdVacancies ?? 0}
                      </td>
              ...
              >
                  ...
                  <div>
                    ...
                    {isCurrentUser ? (
                      ...
                      <select
                        ...
                        className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-700 disabled:opacity-50"
                      >
                        <option value="RECRUITER">Recruiter</option>
                        <option value="ADMIN">Admin</option>
                      </select>
                ...
                {!isCurrentUser ? (
                  <button
                    ...
                    disabled={isUpdating}
                    onClick={() => toggleStatus(user)}
                    className={`mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg border px-3 py-2.5 text-sm font-medium disabled:opacity-50 ${
                      user.status === "ACTIVE"
                        ? "border-red-200 text-red-700 hover:bg-red-50"
                        : "border-green-200 text-green-700 hover:bg-green-50"
                    }`}
                  >
                    {user.status === "ACTIVE" ? (
                      ...
                      <>
                        <UserRoundCheck size={16} /> Activate
                      </>
                    )}
                  </button>
                ) : (
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Create a RecruitX staff account.
                </p>
              </div>
